"""
This version is rendered using the CPU. The functionality is complete, but it is laggy and has high latency.
"""
...
def plot_volume(ax, x, y, Z_top, Z_bot, top_cmap, side_color, ls,
    ...
    ax.plot_surface(X, Y, Z_top, facecolors=rgb_top, rstride=1, cstride=3,
                    antialiased=False, shade=False)

    # ── 辅助：计算用于渐变的归一化高度 ──
    z_min = Z_bot.min()
    z_max = Z_top.max()
    z_range = z_max - z_min + 1e-10  # 防止除零
    ...
    if use_vertical_gradient and top_color is not None and bottom_color is not None:
        grad_cmap = LinearSegmentedColormap.from_list(
            "water_gradient",
            [bottom_color, top_color],  # ← 改成 [深色, 浅色]
            N=256
        )
    else:
        grad_cmap = None
    ...
    def plot_side(X_side, Y_side, Z_side, default_color):
        if use_vertical_gradient and grad_cmap is not None:
            n_z = 10  # ← 可调，20~50 都行，越高越平滑
            # Z_side 形状永远是 (2, N)，第0行=底部，第1行=顶部
            z_bot = Z_side[0, :]
            z_top = Z_side[1, :]

            # 生成精细 Z 网格 (n_z, N)
            frac = np.linspace(0, 1, n_z)[:, np.newaxis]  # (n_z, 1)
            Z_fine = z_bot * (1 - frac) + z_top * frac  # (n_z, N)
            ...
            # 【关键修正】颜色顺序反过来：深海→浅水
            facecolors = grad_cmap(z_norm)  # (n_z, N, 4)

            # X、Y 也要扩展成 (n_z, N) —— 侧面在垂直方向上是常数
            X_fine = np.tile(X_side[0, :], (n_z, 1))
            Y_fine = np.tile(Y_side[0, :], (n_z, 1))

            ax.plot_surface(X_fine, Y_fine, Z_fine,
                            facecolors=facecolors,
                            shade=True, antialiased=False, alpha=0.92,
                            rstride=1, cstride=1)  # 可加 rstride/cstride 控制精度
        else:
            # 原有的非渐变逻辑保持不变
...
class SimulationApp(QMainWindow):
    ...
    def recompute_physics(self):
        ...
        self.k_values = np.linspace(0.01, 50, 200)

        self.x_water = np.linspace(0, L1, 150)
        self.x_shelf = np.linspace(-L1, 0, 150)
    ...
    def update_frame(self):
        ...
        # 海水渐变参数（可调）
        water_top_color = "#4C8BF5"  # 较浅的蓝绿色（接近水面）
        water_bottom_color = "#001020"  # 很深的蓝黑
        ...
        plot_volume(
            ...
            top_cmap="bone",
            side_color=ice_side_color,
            ls=self.light_source,
            use_vertical_gradient=False  # 冰下不渐变
        )

        # 2. 冰架下方的海水（保持单一深色，不渐变）
        plot_volume(
            ax=self.ax, x=self.x_shelf * self.L_scale, y=self.y_range,
            Z_top=Z_top_shelf - ice_vis_thick,
            Z_bot=z_base,
            top_cmap="GnBu_r",
            side_color=deep_water_color,
            ls=self.light_source,
            use_vertical_gradient=False  # ← 关键：关闭渐变
        )
        ...
        plot_volume(
            ...
            Z_top=Z_top_water,
            Z_bot=z_base,
            top_cmap="GnBu_r",  # 顶面仍用 colormap
            side_color=None,  # 不再用单一颜色
            ls=self.light_source,
            use_vertical_gradient=True,
            top_color=water_top_color,
            bottom_color=water_bottom_color
        )
